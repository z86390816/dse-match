// 生成小紅書封面圖（簡體、3:4、1080×1440）→ ig/*.png
// 對應三篇筆記：①計分方法 ②幾多分入U ③M1/M2 算不算分
import sharp from 'sharp';
import fs from 'fs';

const W = 1080, H = 1440;
const F = '"Microsoft YaHei","Microsoft JhengHei","PingFang SC","Noto Sans CJK SC",sans-serif';
fs.mkdirSync('ig', { recursive: true });
...
async function render(name, palette, inner) {
  const svg = `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${W / 2}" y="${H - 108}" font-family='${F}' font-size="30" fill="#e0ecff" text-anchor="middle" letter-spacing="2">JUPAS Calculator · 免費</text>
  </svg>`;
  await sharp(Buffer.from(svg)).png().toFile(`ig/${name}.png`);
  console.log('→ ig/' + name + '.png');
}
...
await render('01_scoring', ['#1e3a8a', '#2563eb', '#3b82f6'], `
  ...
  <text x="70" y="510" font-family='${F}' font-size="150" font-weight="800" fill="#bfdbfe">計分方法</text>
  <text x="70" y="600" font-family='${F}' font-size="52" font-weight="700" fill="#eaf2ff">港三大唔一樣！你計啱咗未？😵</text>
  <g filter="url(#sh)">
    <rect x="70" y="700" width="940" height="470" rx="34" fill="#ffffff"/>
    ${row(740, '港大 HKU', '英×1.5 · 數×1.5 · 計 M1/M2', '#2563eb')}
    ${row(880, '中大 CUHK', '加權 · 5** 當 8.5 分', '#16a34a')}
    ${row(1020, '科大/城大/理大', 'Best 5 / Best 6 + 指定科', '#ea580c')}
  </g>`);

/* ── 封面 2：幾多分入 U（翠绿）── */
await render('02_scores', ['#065f46', '#0d9488', '#22c55e'], `
  ${chip(70, 96, 300, 66, '📊 收藏備用')}
  <text x="70" y="330" font-family='${F}' font-size="128" font-weight="800" fill="#fff">幾多分</text>
  <text x="70" y="470" font-family='${F}' font-size="128" font-weight="800" fill="#d1fae5">先入到 U？</text>
  <text x="70" y="558" font-family='${F}' font-size="50" font-weight="700" fill="#eafff4">349 個專業收生中位數 一覽</text>
  <g filter="url(#sh)">
    <rect x="70" y="660" width="940" height="560" rx="34" fill="#ffffff"/>
    ${score(700, '🏥 醫科 / 牙科', '40+', '#dc2626')}
    ${score(786, '💼 商科 / 環球商業', '40+', '#ea580c')}
    ${score(872, '⚖️ 法律', '高分', '#ca8a04')}
    ${score(958, '💻 計算機 / AI / 數據', '↑ 熱門', '#2563eb')}
    ${score(1044, '🧪 理學 / 工程', '選擇多', '#0d9488')}
    ${score(1130, '📚 文社科 / 教育', '較親民', '#16a34a')}
  </g>`);

/* ── 封面 3：M1/M2 算不算分（紫罗兰）── */
await render('03_m1m2', ['#4c1d95', '#7c3aed', '#a855f7'], `
  ${chip(70, 96, 330, 66, '🧠 港大計分冷知識')}
  <text x="70" y="360" font-family='${F}' font-size="150" font-weight="800" fill="#fff">M1/M2</text>
  <text x="70" y="510" font-family='${F}' font-size="140" font-weight="800" fill="#e9d5ff">計唔計分？</text>
  <text x="70" y="602" font-family='${F}' font-size="52" font-weight="700" fill="#f3e8ff">讀咗兩年，千祈唔好蝕底 😱</text>
  <g filter="url(#sh)">
    <rect x="70" y="700" width="940" height="470" rx="34" fill="#ffffff"/>
    ${row(740, '① 當一科計', '港大理科/工程/商科，仲會加權', '#7c3aed')}
    ${row(880, '② 數學 或 M1/M2', '二揀一，邊科高用邊科', '#2563eb')}
    ${row(1020, '③ 只當普通選修', '部分科唔計——逐科睇清楚！', '#dc2626')}
  </g>`);

console.log('完成：3 张小红书封面已生成于 data-pipeline/ig/');
