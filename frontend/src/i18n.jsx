...
const STRINGS = {
  ...
  filterScopeTitle: { zh: '範圍', en: 'Scope' },
  filterSortTitle: { zh: '排序', en: 'Sort' },
  filterUniTitle: { zh: '院校', en: 'School' },
  scopeAll: { zh: '全部', en: 'All' },
  scopeAttainable: { zh: '入到', sc: '进得了', en: 'Can get in' },
  scopeOutOfReach: { zh: '入唔到', sc: '进不了', en: 'Out of reach' },
  scopeReference: { zh: '僅供參考', en: 'Reference' },
  sortMedianDesc: { zh: '收生分 高→低', sc: '收生分 高→低', en: 'Score high→low' },
  sortMedianAsc: { zh: '收生分 低→高', sc: '收生分 低→高', en: 'Score low→high' },
  sortMatch: { zh: '最易入到', sc: '最容易进', en: 'Easiest first' },
  sectionAttainable: { zh: '✅ 入到——有機會取錄', sc: '✅ 进得了——有机会录取', en: '✅ Within reach' },
  sectionOutOfReach: { zh: '❌ 入唔到——分數未夠', sc: '❌ 进不了——分数不够', en: '❌ Out of reach' },
  sectionReference: { zh: 'ℹ️ 僅供參考——此校計分方式未能精確比對', sc: 'ℹ️ 仅供参考——此校计分方式未能精确比对', en: 'ℹ️ Reference only — scoring cannot be replicated' },
  sectionAttainableEmpty: { zh: '以目前成績，這裡未有配得上的專業。', sc: '以目前成绩，这里没有配得上的专业。', en: 'No programmes match your current grades here.' },
  sectionOutOfReachEmpty: { zh: '沒有落在你分數以外的專業。', sc: '没有落在你分数以外的专业。', en: 'No out-of-reach programmes here.' },
  sectionReferenceEmpty: { zh: '沒有無法比對的專業。', sc: '没有无法比对的专业。', en: 'No reference-only programmes here.' },
  loadMore: { zh: '載入更多', en: 'Load more' },
  gapPctNote: { zh: '＊各校計分尺度不同（例如 PolyU 約 200 分制），「收生分 高→低」只反映該校自己的分數高低，跨校比難度請用「最易入到」或看「距中位數」括號內的百分比。', sc: '＊各校计分尺度不同（例如 PolyU 约 200 分制），「收生分 高→低」只反映该校自己的分数高低，跨校比难度请用「最容易进」或看「距中位数」括号内的百分比。', en: '* Scoring scales differ by institution (PolyU uses a ~200-point scale), so “Score high→low” ranks within each institution’s own scale. To compare difficulty across institutions use “Easiest first”, or read the bracketed percentage next to “vs median”.' },
  ...
  // 未符核心科要求
  reqNeedLevel: { zh: '需達 Level', sc: '需达 Level', en: 'needs Level' },
  reqYours: { zh: '你：', en: 'yours:' },
  reqCsd: { zh: '公民與社會發展需「達標」', sc: '公民与社会发展需「达标」', en: 'Citizenship & Social Development must be Attained' },
  ...
  industryNow: { zh: '行業現狀', en: 'Industry Today' },
  futureOutlook: { zh: '未來前景', en: 'Future Outlook' },
