...
const STRINGS = {
  ...
  sectionOutOfReach: { zh: '❌ 入唔到——分數未夠（差距由細到大）', sc: '❌ 进不了——分数不够（差距由小到大）', en: '❌ Out of reach (smallest gap first)' },
  ...
  gapPctNote: { zh: '＊「距中位數」括號內為相對百分比；各校計分尺度不同（例如 PolyU 約 200 分制），跨校比較請以百分比為準。', sc: '＊「距中位数」括号内为相对百分比；各校计分尺度不同（例如 PolyU 约 200 分制），跨校比较请以百分比为准。', en: '* The bracketed figure next to “vs median” is the relative gap. Scoring scales differ by institution (PolyU uses a ~200-point scale), so compare across institutions by percentage.' },
